...
const parseAmazonProduct = async (iid = 'B01HVI1C46') => {
  cl('we start parsing data for iid:', iid);
  ...
  const getData = async () => {
    ...
    const html = await getHtmlBody(url + iid, true);
    //console.log('data: ', html);
    const $ = cheerio.load(html);
    const dp = $('#imageBlock_feature_div'); //('#dp');
    const pageCountry = $('#glow-ingress-line2');
    console.log(`Curent country is: ${pageCountry.text()}`);
